...
const TRUST_RULES = [
  // ── Tier 1: unconditional ────────────────────────────────────────────────
  {
    ...
    rec: 'Name the individuals behind the business',
    detail: 'Replace collective references ("our team", "our staff") with named individuals and their roles.'
  },
  {
    ...
    rec: 'Anchor the unverified claims',
    detail: 'Give each claim something checkable beside it — a founding year, a project count, a licence number, or a link to the source.'
  },
  {
    ...
    rec: 'Complete the Organization schema entity',
    detail: 'Add legal name, address, contact, and sameAs pointing at profiles the business controls.'
  },
  {
    ...
    rec: 'Surface a visible postal address',
    detail: 'Put the full address in rendered HTML where a visitor can read it, not only in schema.'
  },
  ...
  // ── Tier 2: gated on client configuration ────────────────────────────────
  {
    ...
    rec: 'Add Person schema for the byline',
    detail: 'Link it to a populated author page via sameAs, and reference it as author from the Organization graph.'
  },
  {
    ...
    rec: 'Build an author bio page',
    detail: 'Carry the author\'s credentials on it, and link every byline to it.'
  },
  {
    ...
    rec: 'Display reviews and add Product review markup',
    detail: 'Star rich results ARE achievable on this entity type, so scope the markup to the Product.'
  },
  {
    ...
    rec: 'Display reviews for conversion',
    detail: 'Surface collected reviews on the page where a visitor decides.',
  ...
  },
  {
    ...
    rec: 'Surface practitioner credentials',
    detail: (s, p) => 'Show licensure, the credentialing body, and jurisdiction.'
  ...
  },
  {
    ...
    rec: 'Link to independent verification',
    detail: 'Point at a licensing registry, professional association, review platform, or press coverage.'
  }
];
...
function evaluateTrustRules(signals, client) {
  ...
  TRUST_RULES.forEach(rule => {
    ...
    fired.push({
      ...
      detail: typeof rule.detail === 'function' ? rule.detail(s, p) : (rule.detail || ''),
      impact: rule.impact,
      effort: rule.effort,
